...
def get_linear_gpr_model(model_object, max_flux=1e4, gene_drain_prefix='GXD_', gene_metab_prefix='GXM_',
    ...
    rx_g_drains = [gene_drain_prefix + g for g in gene_names]
    rx_and_comp = [and_component_prefix + str(i) for i in range(len(int_nodes_list))]
    ordered_or_comps = [[''.join([or_component_prefix, rx, '_' + str(i)]) for i in range(len(rx_to_node[rx]))]
                        for rx in rxd_list_ordered]

    rx_or_comp = list(chain(*ordered_or_comps))
    rx_or_comp_dict = dict(zip(rxd_list_ordered, ordered_or_comps))
    rx_conv_id = [reaction_conv_prefix + rx for rx in rxd_list_ordered]

    rx_identifiers = rx_g_drains + rx_and_comp + rx_or_comp + rx_conv_id

    met_g_exp = [gene_metab_prefix + g for g in gene_names]
    met_or_comp = [gpr_component_metab_prefix + str(i) for i in range(len(int_nodes_list))]
    met_rx_ids = [reaction_metab_prefix + r for r in rxd_list_ordered]

    rx_to_node_out = {k: ([list(i) for i in v], rx_or_comp_dict[k]) for k, v in rx_to_node.items()}
    met_identifiers = met_g_exp + met_or_comp + met_rx_ids
    ...
    return (gene_model, full_mat, rx_to_node_out) if return_gpr_map else (gene_model, full_mat)
...
def get_integrated_gpr_model(model: ConstraintBasedModel, max_flux=1e4, reaction_metab_prefix='RXM_',
    ...
    orig_metabolites, orig_metabolic_fluxes = list(integrated_irrev_model.metabolite_names), \
                                              list(integrated_irrev_model.reaction_names)
    M, N = [len(x) for x in [integrated_irrev_model.metabolite_names, integrated_irrev_model.reaction_names]]
    gpr_model, _, rx_to_gpr = get_linear_gpr_model(model, max_flux=max_flux, return_gpr_map=True)
    ...
    return integrated_irrev_model, cb_model_rev_map, mt_to_add, rx_to_gpr, orig_metabolic_fluxes, orig_metabolites
...
def merge_linear_with_causal_model(model: ConstraintBasedModel, graph: GenericGraph, gene_metab_prefix='GXM_',
    ...
    gxd_names, gxd_args = zip(
        *((gene_drain_prefix + g, {gene_metab_prefix + g: 1, gene_pool_prefix + g: -1}) for g in total_genes))
    gxp_names, gxp_args = zip(*((gene_poolrx_prefix + g, {gene_pool_prefix + g: 1}) for g in total_genes))

    model.add_reactions(args=gxd_args + gxp_args, bounds=[(0, max_flux)] * (len(gxd_args) + len(gxp_args)),
                        names=gxd_names + gxp_names)
...
class IntegratedGPRModel(ConstraintBasedModel):
    def __init__(self, model: ConstraintBasedModel, solver=None, gpr_rx_bound=1e4, optimizer=False, **kwargs):
        integrated_irrev_model, cb_model_rev_map, mt_to_add, rx_to_gpr, metabolic_fluxes, metabolites = \
            get_integrated_gpr_model(model, gpr_rx_bound, **kwargs)
        print('Integrated GPR model', integrated_irrev_model.get_stoichiometric_matrix().shape)

        self.metabolic_fluxes = tuple(metabolic_fluxes)
        self.metabolic_rev_map = cb_model_rev_map
        self.gpr_mapping = rx_to_gpr
        self.metabolic_compound_names = tuple(metabolites)
